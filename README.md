...
Brisa `v1.2.1` includes batched SQLite history persistence while retaining the reviewed authentication and security hardening. Use the immutable `ghcr.io/harrentheblack/brisa:1.2.1` image tag for production deployments rather than `latest`. Authentication remains optional for trusted LAN/backwards-compatible deployments, but public exposure requires authentication and network controls. `v1.0.2` remains the stable-drive-ID release.
